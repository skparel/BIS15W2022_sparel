...
```{r}
trophic_masses <- homerange %>% 
  select(common.name, trophic.guild, log10.mass) %>% 
  group_by(trophic.guild) 
  
trophic_masses %>% 
  summarize(min = min(log10.mass),
            median = median(log10.mass),
            max = max(log10.mass))
```
...
carn_mammals %>% 
  ggplot() + 
    geom_boxplot(aes(x = family, y = log10.mass)) +
    coord_flip() +
    theme_minimal()
```

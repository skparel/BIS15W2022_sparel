...
The homerange data set is tidy and has NAs in many variables.
```{r}
homerange %>% head()
```
...
homerange_mammals %>% 
  ggplot() +
  geom_bar(aes(x = trophic.guild)) + # add color w/ fill()
  theme_minimal()
```
...
homerange_mammals %>% 
  top_n(-10, log10.mass) %>% 
  ggplot +
  geom_col(aes(x = common.name, y = log10.mass)) +
  coord_flip() +
  theme_minimal()
```

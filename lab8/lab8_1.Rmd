...
These data are untidy because the observations are grouped in a single row by religion and the column names are variables.
...
plant_data %>% head()
```
This data set is not tidy because the column names are variables.

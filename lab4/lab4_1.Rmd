...
The mammals data set does not contain NAs recorded as NA. However, the negative average of `max. life` shows that missing values are recorded as `-999` in the data set.
```{r}
mammals %>% is.na() %>% sum()
mammals$`max. life` %>% mean()
```
...
```{r}
mammals <- mammals %>% rename(genus = Genus, afr = AFR, `max life` = `max. life`)
```

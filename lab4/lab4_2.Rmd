...
```{r}
mammals %>% filter(family == "Bovidae", mass > 450000)
```
...
```{r}
mammals %>% select(family, genus, species, mass, gestation)
```
...
```{r}
primate <- mammals %>% filter(genus == "Lophocebus" | genus == "Erythrocebus" | genus == "Macaca") %>% 
  select(genus, species, mass, gestation, newborn)
```

The maura species has the smallest `newborn` mass.
```{r}
primate %>% filter(newborn == min(newborn))
```

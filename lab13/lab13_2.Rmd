...
ui <- fluidPage(
    ...
    sliderInput("pointsize", "Select the Point Size", min = 1, max = 3, value = 2, step = 0.5),
  ...
  dashboardBody(
    fluidRow(
      box(title = "Population", width = 3,
          selectInput("pop", "Select a wolf population", choices = unique(wolves$pop),
                      selected = "AK.PEN")),
      box(title = "Sexual composition of wolf packs",
          plotOutput("popPlot"))
    )
  )
)
  ...
  output$popPlot <- renderPlot({
    wolves %>% 
      filter(pop == input$pop,
             !is.na(sex)) %>% 
      ggplot() +
        geom_bar(aes_string(x = "sex", fill = "sex"), position = "dodge") +
        labs(x = NULL,
             y = "Count",
            fill = "Sex") +

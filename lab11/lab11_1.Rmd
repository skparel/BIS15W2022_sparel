...
ggplot(data = homerange) +
  geom_histogram(aes(x = log10.hra), fill = "coral", color = "black", alpha = 0.4) +
  theme_minimal()
```
...
```{r}
ggplot(data = homerange) +
  geom_density(aes(x = log10.hra), color = "red", fill = "coral", alpha = 0.4) +
  theme_minimal()
```
...
```{r}
ggplot(data = homerange) +
  geom_histogram(aes(x = log10.hra, y = ..density..), fill = "coral", color = "black", alpha = 0.4) +
  geom_density(aes(x = log10.hra, y = ..density..), color = "red") +
  theme_minimal()
```
...
```{r}
homerange <- homerange %>% 
  mutate(range_category = case_when(log10.hra <= 3.7 ~ "very_small",
                                    log10.hra > 3.7 & log10.hra <= 4.6 ~ "small",
                                    log10.hra > 4.6 & log10.hra <= 6.1  ~ "medium",
                                    log10.hra > 6.1 ~ "large"))
```
...
```{r}
ggplot(data = homerange) +
  geom_bar(aes(x = range_category, fill = class), position = "dodge") +
  theme_minimal()
```
...
homerange %>% 
  ...
  ggplot() +
    geom_boxplot(aes(x = class, y = log10.mass, fill = class)) +
    theme_minimal()
```

...
ggmap(map2) + 
  geom_point(data = spiders, aes(longitude, latitude), color = "magenta", alpha = 0.4) +
  labs(x = "Longitude", y = "Latitude", title = "Spider Locations")
```
...
```{r}
caves <- spiders %>% slice(which(str_detect(locality, "Cave")))

`%notin%` <- Negate(`%in%`)
spiders <- spiders %>% 
  mutate(cave = case_when(accession %in% caves$accession ~ "Cave",
                          accession %notin% caves$accession ~ "Surface"))
```
...
ggplot() +
  geom_sf(data = ca_comp, size = 0.125)+
  geom_sf(data = ca_cnty_comp, size = 0.125)+
  geom_point(data = spiders, aes(longitude, latitude, color = cave), alpha = 0.4)+
  labs(x = "Figure 1: Distribution of Usofila spiders",
       color = "Location")+
  theme_minimal() +
  theme(axis.text = element_blank()) +
